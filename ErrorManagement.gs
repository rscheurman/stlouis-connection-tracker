//-------------Global Variables---------------------
// Firestore Auth
var email = 'gas-firestore@error-management.iam.gserviceaccount.com';
var key = "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
var projectId = 'error-management'
var fs = FirestoreApp.getFirestore(email, key, projectId);
var unixValue = Date.now()
var currDateTime = Utilities.formatDate(new Date(), "CST", "d MMM yyyy HH:mm:ss")

var currUserEmail = 'null'
//------------End Global Variables---------------
var projectType = 'Google App Script'

var projectName = 'Connection Tracker'

var errorNotifEmail = '500366540@missionary.org'

//Error Class
class Error {
  constructor(type, message, insideFunction) {
    this.type = type;
    this.message = message;
    this.insideFunction = insideFunction;
  }
  init() {
    //send error to Firestore
    // sendNotifEmail(projectName, 'null')
    fs.createDocument(`projects/${projectName}/errors/${this.type + unixValue}`, {errorType: this.type, errorMessage: this.message, insideFunction: this.insideFunction, userEmail: currUserEmail, timeStamp: currDateTime, emailNotifState: false, container: SpreadsheetApp.getActiveSpreadsheet().getName(), unixTs: unixValue})
  }
}

function logError(type, message, insideFunction) {
  var err = new Error(type, message, insideFunction)
  err.init();
}

function sendNotifEmail(project, userEmail) {
  MailApp.sendEmail(errorNotifEmail, `${project} Has a New Error`, `The project ${project} has a newly logged error. The user 'unknown' was using the application when the error occured. Date error occurred: ${new Date()}`, {
    name: 'Error Management Service',
  })
}

function initializeErrorManagement() {
  Logger.log('Initializing Project...')
  fs.updateDocument(`projects/${projectName}`, {projectName: projectName, projectType: projectType, lastMajorUpdate: String(new Date())})
  Logger.log('Initialize SUCCESSFUL')
}

function test() {
  logError('Test Error', 'This is a test error', 'notrealfunc')
}

function timestampFix() {
  // get the ids of all the errors in the db
  var errorsToFix = fs.getDocuments('projects/Connection Tracker/errors');
  errorsToFix.forEach(e => {
    var path = e.name
    // console.log(path)
    var id = path.replace('projects/error-management/databases/(default)/documents/projects/Connection Tracker/errors/', "")
    Logger.log(id)
  })
  // get the correct unix timestamp

  // update record with new unix timestamp

  // repeat
}


















