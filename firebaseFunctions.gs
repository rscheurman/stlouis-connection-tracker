...
function cacheDdUpdate(e) {
  ...
  if(rowExistsInCache(e.range.getRow()) == false) {
    ...
    logError(f.name, f.message, 'cacheDbUpdate')
  }
}
...
function updateRecord(fsResults, row) {
  ...
  try {
    ...
    } [{'name': dataToUpdate.name, 'area': dataToUpdate.areaContact, 'date': dataToUpdate.date}]
  }
  } catch(e) {
...
function createFirestoreRecord(row) {
  ...
  var algoliaID = createAlgoliaRecords([{'name': newData.name, 'firestorePath': getFirestorePath(newData.name), 'area': newData.areaContact, 'date': newData.date}])
